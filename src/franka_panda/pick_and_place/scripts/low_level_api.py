...
if __name__ == "__main__":
    ...
    moveit_control.go_to_pose_goal(0.6, 0.1, 0.7, 0 + pi/4, 0, pi) # yaw - pi/4, _, roll
    
    
    moveit_control.go_to_pose_goal(0.45, 0.1675, 0.55, 0.0 + pi/4, 0, pi) # yaw - pi/4, _, roll
    ...
    finger1_data = Float64()
    finger1_data.data = 0.002
    finger2_data = Float64()
    finger2_data.data = 0.002
    finger1_pub.publish(finger1_data)
    finger2_pub.publish(finger2_data)
    ...
    rospy.sleep(1)
    moveit_control.go_to_pose_goal(0.435, 0.17, 0.9, 0 + pi/4, 0, pi)
    moveit_control.go_to_pose_goal(0.6, -0.2, 0.6, 0 + pi/4, 0, pi) # yaw - pi/4, _, roll
